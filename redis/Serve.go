...
func Serve(_serviceName string, f fn) func() {
	...
	loop := func() {
		...
		for true {
			...
			if log, num := cnt(); log {
				fmt.Println(time.Now().String()[11:19] + " service " + serviceName + " rcved " + strconv.Itoa(num) + " tasks")
			}
		}
	}
	go loop()
	return func() {
		fmt.Println("service " + serviceName + "is running")
	}
}
