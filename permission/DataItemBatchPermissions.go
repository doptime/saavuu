...
func LoadDataItemBatchPermissionsFromRedis() (err error) {
	...
	if err != nil {
		logger.Lshortfile.Println("loading DataItemBatchPermissions  error: " + err.Error() + ". Consider Add hash item  DataItemBatchPermissions in redis,with key redis key before ':' and value as permitted batch operations seperated by ','")
		return err
	}
