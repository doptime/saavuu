...
func (db *Ctx[v]) ZRangeWithScores(start, stop int64) (members []v, scores []float64, err error) {
	cmd := db.Rds.ZRangeWithScores(db.Ctx, db.Key, start, stop)
	return db.UnmarshalRedisZ(cmd.Val())
}
func (db *Ctx[v]) ZRevRangeWithScores(start, stop int64) (members []v, scores []float64, err error) {
...
func (db *Ctx[v]) ZRangeByScore(opt *redis.ZRangeBy) (out []v, err error) {
	cmd := db.Rds.ZRangeByScore(db.Ctx, db.Key, opt)

	return db.UnmarshalToSlice(cmd.Val())
}
func (db *Ctx[v]) ZRangeByScoreWithScores(opt *redis.ZRangeBy) (out []v, scores []float64, err error) {
	cmd := db.Rds.ZRangeByScoreWithScores(db.Ctx, db.Key, opt)
	return db.UnmarshalRedisZ(cmd.Val())
}
func (db *Ctx[v]) ZRevRangeByScore(opt *redis.ZRangeBy) (out []v, err error) {
	cmd := db.Rds.ZRevRangeByScore(db.Ctx, db.Key, opt)
	return db.UnmarshalToSlice(cmd.Val())
}
func (db *Ctx[v]) ZRevRange(start, stop int64) (out []v, err error) {
	var cmd *redis.StringSliceCmd
...
func (db *Ctx[v]) ZRevRangeByScoreWithScores(opt *redis.ZRangeBy) (out []v, scores []float64, err error) {
...
func (db *Ctx[v]) ZPopMax(count int64) (out []v, scores []float64, err error) {
	cmd := db.Rds.ZPopMax(db.Ctx, db.Key, count)
	return db.UnmarshalRedisZ(cmd.Val())
}
func (db *Ctx[v]) ZPopMin(count int64) (out []v, scores []float64, err error) {
